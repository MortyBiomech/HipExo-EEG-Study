function step15_channel_temporal_and_spectral_landmarkwarp()
% GOAL
...
%   Apply the pre-RHS baseline, then landmark-warp potentials; compute PSD in original time.
...
methodBase = struct('cyclePercent', cfg.cyclePercent, ...
    'timeBaselineWindowMs', cfg.timeBaselineWindowMs, ...
    'timeHalfBandwidth', cfg.timeHalfBandwidth, ...
    'inputToMicrovolt', cfg.inputToMicrovolt, 'analysisVersion', "landmark_timewarp_v1");

%% Analyze each subject

for s = 1:numel(subjects)
    T = M(M.Subject == subjects(s), :);
    commonWarpPercent = common_warp_percent_local(T);
    method = methodBase;
    method.commonWarpPercent = commonWarpPercent;
    ...
    for r = 1:height(T)
        ...
        if isempty(runResult)
            [timeSum, psdSum] = analyze_dataset_local(EEG, cfg, info, commonWarpPercent);
        ...
        if isempty(reference)
            ...
            R.commonWarpPercent = commonWarpPercent;
    ...
    R.timeDescription = sprintf(['Signed potential; per-epoch channel mean baseline ' ...
        'from %g to %g ms relative to RHS; then piecewise RHS-LTO-LHS-RTO-RHS ' ...
        'landmark warping to one common subject template and epoch-weighted averaging.'], ...
        cfg.timeBaselineWindowMs);
...
%% Build one common gait-landmark template per subject

function warpPercent = common_warp_percent_local(T)
    names = {'WarptoRHSms', 'WarptoLTOms', 'WarptoLHSms', ...
        'WarptoRTOms', 'WarptoNextRHSms'};
    assert(all(ismember(names, T.Properties.VariableNames)), ...
        'Step 10 manifest is missing gait-landmark warp columns.');

    warpMs = T{:, names};
    assert(all(isfinite(warpMs), 'all') && all(diff(warpMs, 1, 2) > 0, 'all'), ...
        'Invalid Step 10 gait-landmark warp values.');

    warpPercentRows = 100 * (warpMs - warpMs(:, 1)) ./ ...
        (warpMs(:, end) - warpMs(:, 1));
    warpPercent = median(warpPercentRows, 1);
    warpPercent([1 end]) = [0 100];
    assert(all(diff(warpPercent) > 0), 'Invalid common gait-landmark template.');
end

%% Analyze one run

function [timeSum, psdSum] = analyze_dataset_local(EEG, cfg, info, commonWarpPercent)
    ...
    for tr = 1:EEG.trials
        ...
        % Per-epoch pre-RHS baseline, followed by landmark-based time warping.
        xTime = x - mean(x(:, baselineMask), 2);
        queryMs = interp1(commonWarpPercent, latencies(tr, :), ...
            cfg.cyclePercent, 'linear');
