...
for i = 1:length(files)
    ...
    try
        ...
        for s = 1:nStreams
            ...
            timestampsFinite(s) = ~isempty(ts) && all(isfinite(ts(:)));
            dataFiniteSampled(s) = check_data_finite_sampled(data);
            ...
            if nTimeStamps(s) > 1
                ...
                minDtSec(s) = min(dt);
                maxDtSec(s) = max(dt);
                timestampsStrictlyIncreasing(s) = timestampsFinite(s) && all(dt > 0);

                if isfinite(medianDtSec(s)) && medianDtSec(s) > 0
                    robustGapLimitSec(s) = min(maxTimestampGapSec, max(0.02, 10 * medianDtSec(s)));
                end
            end
        ...
        for s = 1:nStreams
            ...
            streamRow = table( ...
                ...
                'VariableNames', {'FileIndex', 'FileName', 'FullPath', 'StreamIndex', ...
                                  ...
                                  'SampleTimestampMatch', 'TimestampsFinite', ...
                                  'TimestampsStrictlyIncreasing', 'DataFiniteSampled', ...
                                  'RobustGapLimitSec', ...
...
function [usable, status, reason] = check_eeg_usability( ...
    ...
    if abs(nominalSrate - expectedNominalSrate) > absoluteSrateToleranceHz
        status = "BAD_nominal_srate";
        reason = "nominal_sampling_rate_is_too_far_from_expected_500_Hz";
        return;
    end
    ...
    if ~timestampsStrictlyIncreasing || isnan(minDtSec) || minDtSec <= 0
        status = "BAD_nonmonotonic_timestamps";
        reason = "timestamps_are_repeated_or_not_strictly_increasing";
        return;
    end

    if ~dataFiniteSampled
        status = "BAD_nonfinite_data";
        reason = "sampled_EEG_data_contain_NaN_or_Inf";
        return;
    end
    ...
    if isnan(robustGapLimitSec) || robustGapLimitSec <= 0
        status = "BAD_timestamp_gap";
        reason = "robust_timestamp_gap_limit_could_not_be_computed";
        return;
    end

    if maxDtSec > robustGapLimitSec
        status = "BAD_large_timestamp_gap";
        reason = "maximum_timestamp_gap_exceeds_robust_limit";
        return;
    end
