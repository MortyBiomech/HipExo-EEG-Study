...
function STUDY = bemobil_dipoles(STUDY,ALLEEG)
    ...
    clusterNames = lower(string({STUDY.cluster.name}));
    isParentCluster = false(size(clusterNames));
    isParentCluster(1) = true;
    isParentCluster = isParentCluster | contains(clusterNames, 'parent');
    normalizedClusterNames = regexprep(clusterNames, '[^a-zA-Z0-9]', '');
    isOutlierCluster = contains(normalizedClusterNames, 'outlier') | ...
        contains(normalizedClusterNames, 'notclust');
    clsind = find(~isParentCluster & ~isOutlierCluster); 
    ...
    for clust = 1:length(clsind)
        ...
        for k = 1:len 
            ...
            thisICdatasets = STUDY.cluster(clsind(clust)).sets(:,k);
            thisICdatasets = thisICdatasets( ...
                isfinite(thisICdatasets) & ...
                thisICdatasets > 0 & ...
                thisICdatasets <= numel(ALLEEG));
            thisICdatasets = unique(thisICdatasets, 'stable');
            if isempty(thisICdatasets)
                error(['Shared-ICA compatibility: no valid dataset ' ...
                    'index for cluster %d IC column %d.'], ...
                    clsind(clust), k);
            end
        ...
        thisICdatasets = unique(thisICdatasets, 'stable');
        if isempty(thisICdatasets)
            error('Shared-ICA compatibility: no valid dataset index for cluster %d IC column %d.', clsind(clust), ICind);
        end
