...
for s = 1:num_sessions
    ...
    switch has_marker
        ...
        case false
            ...
            plot(GRF.time_stamps, GRF.time_series(Right_leg_indx(1), :), 'LineWidth', 1.2);
            hold on;
            plot(GRF.time_stamps, GRF.time_series(Left_leg_indx(1), :), 'LineWidth', 1.2);
            title(sprintf('Select Stable Walking Phase for %s', current_session));
            xlabel('Time (s)');
            ylabel('GRF Amplitude');
            legend('Right Leg GRF (Ch 1)', 'Left Leg GRF (Ch 1)');
            grid on;
