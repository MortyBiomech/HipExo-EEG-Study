...
all_streams_details = cell(num_sessions, 1); % 用于存储所有EMG信号的具体长度和时间戳
...
for s = 1:num_sessions
    ...
    try
        ...
        emg_lengths = [];
        emg_names   = {};
        emg_first_ts = []; % 新增：用于临时存储 first timestamps
        emg_chans   = 0;
        stream_count = 0;
        ...
        for k = 1:length(streams_raw)
            if strcmp(streams_raw{k}.info.type, 'EMG')
                ...
                % 提取第一个时间戳，增加安全判断防止空数组报错
                if n_points > 0
                    first_timestamp = streams_raw{k}.time_stamps(1);
                else
                    first_timestamp = NaN;
                end
                ...
                emg_first_ts(end+1) = first_timestamp; % 记录第一个时间戳
        ...
        if stream_count > 0
            ...
            % 更新：将 first_timestamps 也保存到 struct 中
            all_streams_details{s} = struct('names', {emg_names}, 'lengths', {emg_lengths}, 'first_ts', {emg_first_ts});
...
%% 6. Detailed Report of All EMG Streams per Session
disp(' ');
disp('========================================================================================');
...
for s = 1:num_sessions
    if EMG_Streams(s) > 0
        ...
        for i = 1:length(details.names)
            % 格式化输出，对齐显示流名称、点数以及 First Timestamp
            fprintf('      - %-30s : %8d points | First Timestamp: %.4f\n', ...
                    details.names{i}, details.lengths(i), details.first_ts(i));
        end
        fprintf('----------------------------------------------------------------------------------------\n');
    end
end
