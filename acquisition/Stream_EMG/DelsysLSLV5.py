...
class InteractiveLSLRunner:
    ...
    def step3_configure(self):
        ...
        try:
            ...
            # === MODIFIED: create the ONE combined EMG stream, now that
            # every sensor's channels are collected. All Delsys EMG channels
            # run at the same rate when the sensors share a mode; if they do
            # not, the mismatch is reported loudly (a single stream cannot
            # carry two rates - fix the sensor modes, not this code).
            if self.emg_combined['guids']:
                rates = sorted({m['rate'] for m in self.emg_combined['meta']})
                if len(rates) > 1:
                    print(f"  -> WARNING: EMG sample rates differ across sensors: {rates}. "
                          f"Using {rates[0]}; align the sensor modes before recording.")
                emg_rate_all = rates[0]
                n_ch = len(self.emg_combined['guids'])
                info_emg = StreamInfo('Delsys_EMG', 'EMG', n_ch, emg_rate_all,
                                      'float32', 'delsys_emg_combined')
                ...
                channels_xml = desc.append_child("channels")
                for k in range(n_ch):
                    m = self.emg_combined['meta'][k]
                    ch = channels_xml.append_child("channel")
                    ch.append_child_value("label", self.emg_combined['labels'][k])
                ...
                self.emg_combined['outlet'] = StreamOutlet(info_emg)
                print(f"  -> LSL Stream Ready: [Delsys_EMG] | {n_ch} channels | {emg_rate_all} Hz")
                for k, lb in enumerate(self.emg_combined['labels']):
        ...
        try:
            while self.is_streaming:
                if self.aero.CheckDataQueue():
                    ...
                    ec = self.emg_combined
                    if ec['outlet'] and ec['guids']:
                        first_guid = ec['guids'][0]
