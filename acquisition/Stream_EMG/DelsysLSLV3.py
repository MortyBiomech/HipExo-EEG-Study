    ...
    def step3_configure(self):
        ...
        try:
            ...
            for idx in range(len(sensor_names)):
                ...
                print(f"  -> Scanning {len(ch_info)} channels for Sensor: {s_name} (Pair: {pair_num})...")
                ...
                # LSL Outlet Creation
                if emg_guids:
                    # Naming format: Delsys_[Type]_Pair[#]_EMG
                    stream_name = f'Delsys_{s_name}_Pair{pair_num}_EMG'
                    source_id = f'delsys_{s_name.lower()}_p{pair_num}_emg'
                ...
                if imu_guids:
                    # Naming format: Delsys_[Type]_Pair[#]_IMU
                    stream_name = f'Delsys_{s_name}_Pair{pair_num}_IMU'
                    source_id = f'delsys_{s_name.lower()}_p{pair_num}_imu'
